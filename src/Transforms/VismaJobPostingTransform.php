...
class VismaJobPostingTransform implements AbstractDataTransform
{
	...
	/**
	 * Visma may leave EmploymentDuration/Descr empty; fall back to Name, list feed, end-date text, or Assignment.EmploymentEndDate.
	 */
	private function extractEmploymentDurationText(SimpleXMLElement $assignment, SimpleXMLElement $singleLoc, SimpleXMLElement $listLoc): string
	{
		$candidates = [
		...
		foreach ($assignments as $assignment) {
			try {
				...
				$jobPosting = Schema::jobPosting()
					...
					->employmentType($employmentGrade ?: (string) ($localization->EmploymentGrade->Name ?? ''))
